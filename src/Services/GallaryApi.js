...
export const fetchGallary = async (searchQuery, page=1)=>{
...
const response = await fetch(`${BASE_URL}${searchQuery}${URL}&page=${page}`)
    if (response.ok) {
      return response.json();
    }
    return await Promise.reject(new ErrorInfo(`NO image ${searchQuery}`))}
